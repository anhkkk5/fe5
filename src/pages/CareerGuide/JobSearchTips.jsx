...
function JobSearchTips() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      console.error("Error fetching job search tips posts:", error);
      setPosts([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="career-guide">
      ...
      >
        <div className="career-guide__banner-inner">
          <h1 className="career-guide__title">Bí kíp tìm việc</h1>
          <p className="career-guide__subtitle">
            Những mẹo và chiến lược hiệu quả để tìm được công việc mơ ước
          </p>
        </div>
      </div>
